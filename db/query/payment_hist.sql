...
UPDATE public.payment_hist
SET user_id=$1, email=$2, "name"=$, value=$, "method"=$, automatic=$, payment_date=$, payment_expireted=$, payment_status=$, currency=$, invoice=$, customer=$, "interval"=$
WHERE id=nextval('payment_hist_id_seq'::regclass);