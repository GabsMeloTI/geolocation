...
const getOneTrailerByUserId = `-- name: GetOneTrailerByUserId :one
...
func (q *Queries) GetOneTrailerByUserId(ctx context.Context, userID int64) (Trailer, error) {
	row := q.db.QueryRowContext(ctx, getOneTrailerByUserId, userID)
	var i Trailer
	err := row.Scan(
		&i.ID,
		&i.LicensePlate,
