...
const getOneTractorUnitByUserId = `-- name: GetOneTractorUnitByUserId :one
...
func (q *Queries) GetOneTractorUnitByUserId(ctx context.Context, userID int64) (TractorUnit, error) {
	row := q.db.QueryRowContext(ctx, getOneTractorUnitByUserId, userID)
	var i TractorUnit
	err := row.Scan(
		&i.ID,
		&i.LicensePlate,
