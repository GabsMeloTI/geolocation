...
const getAdvertisementExist = `-- name: GetAdvertisementExist :one
SELECT id, advertisement_id, route_hist_id, user_id, route_choose, created_at
...
type GetAdvertisementExistParams struct {
	RouteHistID     int64 `json:"route_hist_id"`
	UserID          int64 `json:"user_id"`
	AdvertisementID int64 `json:"advertisement_id"`
}

func (q *Queries) GetAdvertisementExist(ctx context.Context, arg GetAdvertisementExistParams) (AdvertisementRoute, error) {
	row := q.db.QueryRowContext(ctx, getAdvertisementExist, arg.RouteHistID, arg.UserID, arg.AdvertisementID)
	var i AdvertisementRoute
	err := row.Scan(
		&i.ID,
		&i.AdvertisementID,
...
type GetAllAdvertisementByUserRow struct {
	ID                      int64                 `json:"id"`
	UserID                  int64                 `json:"user_id"`
	UserName                string                `json:"user_name"`
	ActiveThere             sql.NullTime          `json:"active_there"`
...
func (q *Queries) GetAllAdvertisementByUser(ctx context.Context, userID int64) ([]GetAllAdvertisementByUserRow, error) {
	rows, err := q.db.QueryContext(ctx, getAllAdvertisementByUser, userID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
	var items []GetAllAdvertisementByUserRow
	for rows.Next() {
		var i GetAllAdvertisementByUserRow
		if err := rows.Scan(
			&i.ID,
			&i.UserID,
			&i.UserName,
...
const updateAdsRouteChooseByUserId = `-- name: UpdateAdsRouteChooseByUserId :exec
UPDATE advertisement_route SET route_choose = $1 WHERE user_id = $2 AND advertisement_id = $3
`

type UpdateAdsRouteChooseByUserIdParams struct {
	RouteChoose     int64 `json:"route_choose"`
	UserID          int64 `json:"user_id"`
	AdvertisementID int64 `json:"advertisement_id"`
}

func (q *Queries) UpdateAdsRouteChooseByUserId(ctx context.Context, arg UpdateAdsRouteChooseByUserIdParams) error {
	_, err := q.db.ExecContext(ctx, updateAdsRouteChooseByUserId, arg.RouteChoose, arg.UserID, arg.AdvertisementID)
	return err
}
