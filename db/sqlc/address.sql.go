...
const findAddressByCEPNew = `-- name: FindAddressByCEPNew :one
select id, lat, lon from addresses_coordinates a where a.cep = $1 limit 1
`

type FindAddressByCEPNewRow struct {
	ID  sql.NullInt64   `json:"id"`
	Lat sql.NullFloat64 `json:"lat"`
	Lon sql.NullFloat64 `json:"lon"`
}

func (q *Queries) FindAddressByCEPNew(ctx context.Context, cep sql.NullString) (FindAddressByCEPNewRow, error) {
	row := q.db.QueryRowContext(ctx, findAddressByCEPNew, cep)
	var i FindAddressByCEPNewRow
...
WHERE
  ...
  AND (
    ...
    regexp_replace(lower(unaccent(s.name)), '[z]', 's', 'gi')
        LIKE '%' || regexp_replace(lower(unaccent($1)), '[z]', 's', 'gi') || '%'
    )

  AND (
    $2 = '' OR
    c.search_vector @@ plainto_tsquery('portuguese', $2)
    )

  AND (
    $3 = '' OR
    st.search_vector @@ plainto_tsquery('portuguese', $3)
    )

  AND (
    $4 = '' OR
    n.search_vector @@ plainto_tsquery('portuguese', $4)
    )
...
const findTwoRandomCEPs = `-- name: FindTwoRandomCEPs :many
SELECT
    cep::text AS cep
FROM public.unique_ceps
ORDER BY RANDOM()
...
func (q *Queries) FindTwoRandomCEPs(ctx context.Context) ([]string, error) {
	rows, err := q.db.QueryContext(ctx, findTwoRandomCEPs)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
const findUniqueAddressByCEP = `-- name: FindUniqueAddressByCEP :many
SELECT
    u.street_id,
    u.street_name,
    u.neighborhood_name,
...
type FindUniqueAddressByCEPRow struct {
	StreetID         int32           `json:"street_id"`
	StreetName       sql.NullString  `json:"street_name"`
	NeighborhoodName sql.NullString  `json:"neighborhood_name"`
	NeighborhoodLat  sql.NullFloat64 `json:"neighborhood_lat"`
...
func (q *Queries) FindUniqueAddressByCEP(ctx context.Context, cep interface{}) ([]FindUniqueAddressByCEPRow, error) {
	...
	defer rows.Close()
	var items []FindUniqueAddressByCEPRow
	for rows.Next() {
		var i FindUniqueAddressByCEPRow
		if err := rows.Scan(
			&i.StreetID,
			&i.StreetName,
			&i.NeighborhoodName,
